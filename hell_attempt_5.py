...
class root():
    ...
    def draw_input_matrix(self):
        ...
        for x in range(columns):
            ...
            dpg.add_group(horizontal=True, width=50, parent=self.inp_parent, tag=f"column {x}")
            for y in range(rows):
    ...
    def clear_input_area(self):
        ...
        self.input_matrix_arr = [[]]
        print('clearing input area')

    def change_value_callback(self, sender):
        _x, _y = sender.split(' ')
        self.input_matrix_arr[int(_x)][int(_y)].append(sender.get_value())
        print('fuck')
